"""
מיפוי תכשירים לקישורי תוויות במאגר משרד החקלאות
...
LABELS = {
    ...
    "מאבריק": {"id": "1821", "ai": "TAU-FLUVALINATE"},
    "אקסירל": {"id": "4001", "ai": "CYANTRANILIPROLE"},
}

def get_label_url(name):
    if name in LABELS:
        info = LABELS[name]
        return {"url": BASE_URL + info["id"], "name": name, "ai": info["ai"]}
    for n, info in LABELS.items():
        if n in name or name in n:
            return {"url": BASE_URL + info["id"], "name": n, "ai": info["ai"]}
    return None
